        ...
        while (1):
            try:
                with sr.Microphone() as source2:
                    ...
                    print("Said " + inpcmd)
                    if inpcmd == "start":
                        t2s("Nice so we can start now")
                        global flag_start
                        flag_start = True
                        print("In thread ", flag_start)
                    else:
                        t2s("you said " + inpcmd + " say again")
                    if inpcmd == "stop":
                        t2s("Nice so we can stop now")
                        global flag_stop
                        flag_stop = False
                        global main_flag
                        main_flag = False
...
AudioThread1 = AudioThread()
AudioThread1.start()
t2s("Welcome! Just say start ,to start the process and stop, to stop")
...
with detection_graph.as_default():
    with tf.Session(graph=detection_graph) as sess:
        while main_flag:
            time.sleep(2)
            print("In while", flag_start, flag_stop)
            while flag_start and flag_stop:
                ...
                # while run:
                # _, frame = camera.read()
                # frame = cv2.imshow('object detection', cv2.resize(image_np, (800, 600)))
